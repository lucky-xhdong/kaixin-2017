(function ($) {
    var Wujiangku = function (o, options) {
        this.o = $("#" + o);
        if (typeof(options) == 'object') {
            this.options = $.extend({}, Wujiangku.options, options);
        }
        this.init();
    };
    Wujiangku.prototype = {
        ...
        draw: function () {
            ...
            $(this.li).eq(prev - 1).attr('class','left').css({
            ...
            $(this.li).eq(prev - 1).attr('class','left').find('.total').css({
                bottom: 24 +'px'
            });
            ...
            $(this.li).eq(this.currentPage - 1).attr('class','current').find('.total').css({
                bottom: 15 + 'px'
            });
            ...
            $(this.li).eq(next - 1).attr('class','right').find('.total').css({
                bottom: 24 +'px'
            });
        },
        bind: function () {
        ...
        createControl: function () {
            var html = '', _this = this;
            html += '<div class="carousel-controls wjk-controls">';
            ...
            $('.wjk-controls').undelegate('.btn-prev', 'click').delegate('.btn-prev', 'click', function () {
                _this.turn('right');
            });
            $('.wjk-controls').undelegate('.btn-next', 'click').delegate('.btn-next', 'click', function () {
                _this.turn('left');
            });
        },
        turnPage: function (page, direction) {
            ...
            var play = function (page, direction, time) {
                ...
                if (prev != _this.currentPage && next != _this.currentPage) {
                    ...
                    $(_this.li).eq(currentPrev - 1).attr('class','left').find('.total').animate({
                        bottom: 24 +'px'
                    }, time);
                    ...
                    $(_this.li).eq(_this.currentPage - 1).attr('class','current').find('.total').animate({
                        bottom: 15+'px'
                    }, time);
                    ...
                    $(_this.li).eq(currentNext - 1).attr('class','right').find('.total').animate({
                        bottom: 24 +'px'
                    }, time);
                }
                if (direction == 'left') {
                    ...
                    $(_this.li).eq(prev - 1).attr('class','left').find('.total').animate({
                        bottom: 24 +'px'
                    }, time);
                    ...
                    $(_this.li).eq(page - 1).attr('class','current').css({zIndex: 3}).find('.total').animate({
                        bottom: 15+'px'
                    }, time);
                    ...
                    $(_this.li).eq(next - 1).attr('class','right').css({zIndex: 2}).find('.total').animate({
                        bottom: 24 +'px'
                    }, time);
                    ...
                    $(_this.li).eq(page - 1).attr('class','current').css({zIndex: 3}, time).find('.total').animate({
                        bottom: 15+'px'
                    }, time);
        ...
        animate: function () {
            ...
            $(_this.li).each(function (index, element) {
                var pngsrc = $(this).data('pngsrc'),gifsrc = $(this).data('gifsrc');
                if ($(this).hasClass('current')) {
                    $(this).find('img').attr('src',gifsrc);
                }else {
                    $(this).find('img').attr('src',pngsrc);
                }
            })
        }
    };
    ...
    window.Wujiangku = Wujiangku;
})(jQuery);
// <!--var curIndex = 0; //时间间隔 单位毫秒-->
// <!--var timeInterval = 200;-->
// <!--var arr = new Array();-->
// <!--arr[0] = "images/qingbubing1.png";-->
// <!--arr[1] = "images/qingbubing2.png";-->
// <!--arr[2] = "images/qingbubing3.png";-->
// <!--arr[3] = "images/qingbubing4.png";-->
